...
public class RangeSettings extends BlackOutModule {
    ...
    public double placeRangeTo(BlockPos pos, Vec3d from) {
        ...
        if (from == null) {
            ...
            switch (placeRangeFrom.get()) {
                case Middle -> from = from.withX((pBB.minX + pBB.maxX) / 2).withY((pBB.minY + pBB.maxY) / 2).withZ((pBB.minZ + pBB.maxZ) / 2);
                case Feet -> from = pPos;
            }
        }
        Vec3d feet = new Vec3d(pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5);
        return switch (placeRangeMode.get()) {
            case NCP -> getRange(from, feet.add(0, 0.5, 0));
            case Height -> getRange(from, feet.add(0, placeHeight.get(), 0));
            case Vanilla -> getRange(from, OLEPOSSUtils.getClosest(mc.player.getEyePos(), feet, 1, 1));
            case CustomBox -> getRange(from, OLEPOSSUtils.getClosest(mc.player.getEyePos(), feet, blockWidth.get(), blockHeight.get()));
        };
    }
    ...
    public double getDistFromCenter(Box bb, Vec3d feet, Vec3d from) {
        ...
        if (from.x == rangePos.x && from.z == rangePos.z) return 0;
        Vec3d dist = new Vec3d(from.x - rangePos.x, 0, from.z - rangePos.z);
        if (getDistXZ(dist) < halfWidth * Math.sqrt(2)) return 0;
        if (dist.getZ() > 0.0) {
            rangePos = rangePos.withX(rangePos.x).withZ(rangePos.z + halfWidth);
        } else if (dist.getZ() < 0.0) {
            rangePos = rangePos.withX(rangePos.x).withZ(rangePos.z - halfWidth);
        } else if (dist.getX() > 0.0) {
            rangePos = rangePos.withX(rangePos.x + halfWidth).withZ(rangePos.z);
        } else {
            rangePos = rangePos.withX(rangePos.x - halfWidth).withZ(rangePos.z);
        }
    ...
    public double miningRangeTo(BlockPos pos, Vec3d from) {
        ...
        Vec3d feet = new Vec3d(pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5);
        return switch (miningRangeMode.get()) {
            case NCP -> getRange(from, feet.add(0, 0.5, 0));
            case Height -> getRange(from, feet.add(0, miningHeight.get(), 0));
            case Vanilla -> getRange(from, OLEPOSSUtils.getClosest(mc.player.getEyePos(), feet, 1, 1));
            case CustomBox -> getRange(from, OLEPOSSUtils.getClosest(mc.player.getEyePos(), feet, closestMiningWidth.get(), closestMiningHeight.get()));
        };
    }
    ...
    public enum PlaceRangeMode { NCP, Height, Vanilla, CustomBox }
    public enum AttackRangeMode { NCP, UpdatedNCP, Height, Vanilla, Middle, CustomBox }
    public enum MiningRangeMode { NCP, Height, Vanilla, CustomBox }
    public enum FromMode { Eyes, Middle, Feet }
}
